...
func _ready():
	...
	for i in Procgen.hyperlanes:
		...
		update_link_assoc_bucket(i.lsys, lane)
		update_link_assoc_bucket(i.rsys, lane)
...
func update_link_assoc_bucket(system_id, link):
	if system_id in link_assoc_buckets:
		var bucket = link_assoc_buckets[system_id]
		if not link in bucket:
			bucket.push_back(link)
	else:
		link_assoc_buckets[system_id] = [link]

func update_for_explore(system_id):
	movement.get_node(system_id).show()
